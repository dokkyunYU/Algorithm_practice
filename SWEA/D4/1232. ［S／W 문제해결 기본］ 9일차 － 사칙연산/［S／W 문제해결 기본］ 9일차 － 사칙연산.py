def postorder(node_num):
    if len(calculate_tree[node_num]) > 1:
        ...
        if calculate_tree[node_num][0] == "+":
            calculate_stack.append(calculate_stack.pop() + calculate_stack.pop())
        elif calculate_tree[node_num][0] == "-":
            calculate_stack.append(-calculate_stack.pop() + calculate_stack.pop())
        elif calculate_tree[node_num][0] == "*":
            calculate_stack.append(calculate_stack.pop() * calculate_stack.pop())
        else:
            calculate_stack.append(1 / calculate_stack.pop() * calculate_stack.pop())
    else:
